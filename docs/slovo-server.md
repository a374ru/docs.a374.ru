...
Далее найти пунк меню и содать шаблон. Заполнить шаблон данными и сохранить его. Позднее из данногоо шаблона можно создавать с уверенностью виртуальную машину. Можно потренироваться и создать несколько шаблонов, чтобы не тратить время на создание и удаление виртуальных машин, далее просто `VM`.
...
Через малое время будет создан экземпляр сервера, которым можно управлять сразу через веб консоль. Для этого вверху страницы найдите кнопку активации `cloud shell` и пере вами откроется консоль `shell`.
...
Впишите такой код в файл `app.yaml` :
