...
<p><a href="#локальный-запуск-сервера-nodejs">Запуск локального сервера <code>NodeJS</code></a></p>
<p><a href="#локальный-запуск-сервера-python">Запуск локального сервера <code>Python</code></a></p>
...
## <span id="sloc-2" style="color: #7C9655;">Локальный запуск сервера `python`
...
## Еще способ запуска сервера с клика по файлу
...
В терминале перейдите папку с созданным ранее файлом `server-loc` и выполните команду:
...
<span style="color: #8F7161; background-color: #dea5; padding:0 1ch">Ваш файл стал выполняемым. Теперь чтобы запустить сервер просто кликните по файлу. Файл не должен иметь расширения `sh`.
